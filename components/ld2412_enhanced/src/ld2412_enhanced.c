/**
 * @file ld2412_enhanced.c
 * @brief Enhanced LD2412C mmWave Radar Driver Implementation
 *
...
static ld2412_state_t s_state = {0};
static uint8_t s_rx_buffer[LD2412_UART_BUF_SIZE];
...
#define LD2412_FRAME_TIMEOUT_MS     200

/* Watchdog: if no valid frames for this long, attempt recovery */
#define LD2412_WATCHDOG_TIMEOUT_MS  5000
...
#define LD2412_STATUS_INTERVAL_MS  10000  /* Log status every 10 seconds */

/* Callbacks */
static ld2412_state_callback_t s_state_callback = NULL;
static ld2412_distance_callback_t s_distance_callback = NULL;
static ld2412_energy_callback_t s_energy_callback = NULL;
static ld2412_gate_callback_t s_gate_callback = NULL;
...
static void flush_and_reset(void) {
    ...
    uart_flush_input(LD2412_UART_NUM);
    reset_frame_parser();
}
...
static void parse_basic_frame(const uint8_t *data, int len) {
    ...
    bool raw_moving = (s_state.target.target_state & LD2412_STATE_MOVING) != 0;
    bool raw_static = (s_state.target.target_state & LD2412_STATE_STATIC) != 0;
    if (raw_moving) {
    ...
    bool prev_raw_moving = (prev_raw_target_state & LD2412_STATE_MOVING) != 0;
    bool prev_raw_static = (prev_raw_target_state & LD2412_STATE_STATIC) != 0;
    ...
    if ((debounced_change || periodic_due) && s_state_callback) {
        if (debounced_change) {
            ESP_LOGI(TAG, "State changed -> M:%d S:%d (debounced, calling callback)",
                     (s_state.target.target_state & LD2412_STATE_MOVING) != 0,
                     (s_state.target.target_state & LD2412_STATE_STATIC) != 0);
        }
...
static void parse_engineering_frame(const uint8_t *data, int len) {
    ...
    // Static target detection (no software cooldown - uses sensor's built-in occupancy timeout)
    s_state.static_detected = (s_state.target.target_state & LD2412_STATE_STATIC) != 0;
    ...
    bool prev_eng_raw_moving = (prev_eng_raw_target_state & LD2412_STATE_MOVING) != 0;
    bool prev_eng_raw_static = (prev_eng_raw_target_state & LD2412_STATE_STATIC) != 0;

    bool state_changed = (raw_moving != prev_eng_raw_moving) ||
                         ((s_state.target.target_state & LD2412_STATE_STATIC) != 0) != prev_eng_raw_static ||
    ...
    if (state_changed && s_state_callback) {
        ESP_LOGI(TAG, "ENG RAW state changed -> M:%d S:%d (calling callback)",
                 raw_moving, (s_state.target.target_state & LD2412_STATE_STATIC) != 0);
        s_state_callback(&s_state);
    }
...
static void process_data_frame(const uint8_t *data, int len) {
    ...
    if (data_type == LD2412_DATA_TYPE_BASIC) {
        parse_basic_frame(data, len);
    } else if (data_type == LD2412_DATA_TYPE_ENGINEERING) {
...
static esp_err_t send_command(uint16_t cmd, const uint8_t *data, int data_len) {
    ...
    // Send
    int written = uart_write_bytes(LD2412_UART_NUM, frame, pos);
    if (written != pos) {
        return ESP_FAIL;
    }
...
static esp_err_t send_command_wait(uint16_t cmd, const uint8_t *data, int data_len, int timeout_ms) {
    ...
    while (!s_response_received) {
        int len = uart_read_bytes(LD2412_UART_NUM, s_rx_buffer, sizeof(s_rx_buffer), 0);
        if (len > 0) {
            bytes_seen += len;
            for (int i = 0; i < len; i++) {
...
static esp_err_t enable_config(void) {
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
        ...
        err = send_command_wait(LD2412_CMD_ENABLE_CONFIG, data, sizeof(data), 250);
        if (err == ESP_OK) {
            return ESP_OK;
        }
...
static esp_err_t end_config(void) {
    return send_command_wait(LD2412_CMD_END_CONFIG, NULL, 0, 100);
}
...
esp_err_t ld2412_init(void) {
    ...
    esp_err_t err = uart_driver_install(LD2412_UART_NUM, LD2412_UART_BUF_SIZE * 2, 0, 0, NULL, 0);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to install UART driver: %s", esp_err_to_name(err));
        return err;
    }

    err = uart_param_config(LD2412_UART_NUM, &uart_config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to configure UART: %s", esp_err_to_name(err));
        return err;
    }

    err = uart_set_pin(LD2412_UART_NUM, LD2412_UART_TX_PIN, LD2412_UART_RX_PIN,
    ...
    ESP_LOGI(TAG, "LD2412 driver initialized (UART%d: TX=%d, RX=%d, baud=%d)",
             LD2412_UART_NUM, LD2412_UART_TX_PIN, LD2412_UART_RX_PIN, LD2412_UART_BAUD);
    return ESP_OK;
}

void ld2412_deinit(void) {
    uart_driver_delete(LD2412_UART_NUM);
    ESP_LOGI(TAG, "LD2412 driver deinitialized");
}
...
void ld2412_process(void) {
    ...
    if (s_in_frame && s_frame_start_time > 0) {
        if ((now - s_frame_start_time) > LD2412_FRAME_TIMEOUT_MS) {
    ...
    if ((now - s_last_status_time) > LD2412_STATUS_INTERVAL_MS) {
    ...
    if (s_state.frame_count > 0 && s_last_valid_frame_time > 0 && now > s_last_valid_frame_time) {
        ...
        if (elapsed > LD2412_WATCHDOG_TIMEOUT_MS) {
            ...
            else if (elapsed > (LD2412_WATCHDOG_TIMEOUT_MS * 2)) {
...
esp_err_t ld2412_enable_engineering_mode(void) {
    ...
    err = send_command_wait(LD2412_CMD_ENABLE_ENGINEERING, NULL, 0, 200);
    if (err == ESP_OK) {
...
esp_err_t ld2412_disable_engineering_mode(void) {
    ...
    err = send_command_wait(LD2412_CMD_DISABLE_ENGINEERING, NULL, 0, 100);
    if (err == ESP_OK) {
...
esp_err_t ld2412_set_all_sensitivity(uint8_t move_sensitivity, uint8_t still_sensitivity) {
...
esp_err_t ld2412_read_firmware_version(void) {
    ...
    err = send_command_wait(LD2412_CMD_READ_FIRMWARE, NULL, 0, 100);
...
esp_err_t ld2412_factory_reset(void) {
    ...
    err = send_command_wait(LD2412_CMD_FACTORY_RESET, NULL, 0, 100);
    if (err == ESP_OK) {
...
esp_err_t ld2412_restart(void) {
    ...
    err = send_command_wait(LD2412_CMD_RESTART, NULL, 0, 100);
    if (err == ESP_OK) {
...
const char* ld2412_state_to_string(uint8_t state) {
    switch (state) {
        case LD2412_STATE_NO_TARGET: return "No target";
        case LD2412_STATE_MOVING: return "Moving target";
        case LD2412_STATE_STATIC: return "Static target";
